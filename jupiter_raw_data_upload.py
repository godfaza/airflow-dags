...
def generate_upload_scripts(prev_task,src_dir,src_file,upload_path,bcp_parameters):
    ...
    scripts_list = ['cp -r /tmp/data/src/. ~/ && chmod +x ~/exec_query.sh && ~/exec_query.sh "{}" {}{}/{}.csv "{}" '.format(x["Extraction"],upload_path,x["Schema"],x["EntityName"],bcp_parameters) for x in queries]
    print(scripts_list)
    return  scripts_list
