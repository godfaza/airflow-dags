...
#           bash_command='echo "{{ ti.xcom_pull(task_ids="test-task") }}"',
        bash_command='cp -r /tmp/data/src/. ~/ && chmod +x ~/exec_query.sh && ~/exec_query.sh "{{ti.xcom_pull(task_ids="extract_db_schema")}}" /user/smartadmin/schema/query_out2.csv "{{ti.xcom_pull(task_ids="get_bcp_parameters")}}"',
            )  
