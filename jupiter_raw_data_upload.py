...
with DAG(
    ...
    upload_tables=BashOperator.partial(task_id="upload_tables", do_xcom_push=True).expand(
       bash_command= generate_upload_script.partial(parameters["MaintenancePath"],"EXTRACT_ENTITIES_AUTO.csv",parameters["UploadPath"],parameters["BcpParameters"]).expand(start_mon),
    )
